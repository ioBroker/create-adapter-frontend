...
class App extends Component {
    constructor(props) {
        ...
        this.steps = [
            {
                name: 'Name and icons',
                render: this.renderStepNames,
                optional: false,
                checkers: [this.checkTitle, this.checkAdapterName, this.checkType, this.checkDescription, this.checkKeywords]
            },
            { name: 'Running properties', render: this.renderStepProperties, optional: false },
            {
                name: 'Administrative',
                render: this.renderStepAdministrative,
                optional: false,
                checkers: [this.checkAuthorName, this.checkAuthorGithub, this.checkAuthorEmail]
            },
        ...
        this.state = {
            ...
            answers: {
                ...
                connectionIndicator: window.localStorage.getItem('connectionIndicator') || 'no',
                keywords: window.localStorage.getItem('keywords') || 'template',
                schedule: window.localStorage.getItem('schedule') || '8 * * * *',
                gitRemoteProtocol: window.localStorage.getItem('gitRemoteProtocol') || 'HTTPS',
    ...
    // returns true if adapter title is invalid
    checkKeywords(text) {
        text = text || this.state.answers.keywords;
        if (!isNotEmpty(text)) {
    ...
    parseTypeText(text) {
        const m = text.match(/\(([^)]+)/);
        if (m) {
            return [text.replace('(' + m[1] + ')', '').trim(), (
                <span className={this.props.classes.typeDesc}>{m[1]}</span>)];
        } else {
            return text;
        }
    }
        ...
        return (<FormControl className={this.props.classes.selectType + ' ' + this.props.classes.textInputs}>
            ...
            >
                ...
                {ADAPTER_TYPES.map(type => (
                    <MenuItem value={type.value}>{this.parseTypeText(type.message)}</MenuItem>))}
            </Select>
    ...
    renderStepNames() {
        ...
        return [
            ...
            (<TextField
                label="Keywords"
                disabled={this.state.requesting}
                error={!isNotEmpty(this.state.answers.keywords)}
                className={this.props.classes.title + ' ' + this.props.classes.textInputs}
                value={this.state.answers.keywords}
                onChange={e => this.updateAnswers('keywords', e.target.value)}
    ...
    renderStepProperties() {
        ...
        return [
            ...
            // schedule
            isAdapter && this.state.answers.startMode === 'schedule' ? [(<TextField
                label="CRON schedule"
                disabled={this.state.requesting}
                error={!isNotEmpty(this.state.answers.schedule)}
                value={this.state.answers.schedule}
                onChange={e => this.updateAnswers('schedule', e.target.value)}
                margin="normal"
            />),
                (<Fab className={this.props.classes.buttonCRON} size="small" color="primary"
                      onClick={() => this.setState({ showCron: true })} aria-label="Edit CRON"><EditIcon/></Fab>)
            ] : null,

            // language
            isAdapter ? (<FormControl
                className={this.props.classes.language + ' ' + this.props.classes.textInputs + ' ' + this.props.classes.paddingTop}>
            ...
            // quotes
            isAdapter ? (<FormControl
                className={this.props.classes.quotes + ' ' + this.props.classes.textInputs + ' ' + this.props.classes.paddingTop}>
            ...
            // es6class
            isAdapter ? (<FormControl
                className={this.props.classes.es6class + ' ' + this.props.classes.textInputs + ' ' + this.props.classes.paddingTop}>
            ...
            // connection
            isAdapter ? (<FormControl
                className={this.props.classes.connection + ' ' + this.props.classes.textInputs + ' ' + this.props.classes.paddingTop}>
                <InputLabel shrink>Do you have connection indication to device or service?</InputLabel>
                <Select
                    disabled={this.state.requesting}
                    value={this.state.answers.connectionIndicator}
                    onChange={e => this.updateAnswers('connectionIndicator', e.target.value)}
                >
    ...
    renderStepAdministrative() {
        return [
            ...
            (<FormControl
                key="gitRemoteProtocol"
                className={this.props.classes.selectType + ' ' + this.props.classes.textInputs}>
                <InputLabel shrink htmlFor="age-label-placeholder">
                    Git link type
                </InputLabel>
                <Select
                    disabled={this.state.requesting}
                    error={this.checkType()}
                    value={this.state.answers.gitRemoteProtocol}
                    onChange={e => this.updateAnswers('gitRemoteProtocol', e.target.value)}
                >
                    <MenuItem value="HTTPS">https</MenuItem>
                    <MenuItem value="SSH">ssh</MenuItem>
                </Select>
                <FormHelperText>Select Git remote protocol</FormHelperText>
            </FormControl>)
        ];
    }
    ...
    getCronDialog() {
        if (this.state.showCron) {
            return (<DialogCron
                key="dialogCron1"
                cron={this.state.answers.schedule}
                onClose={() => this.setState({ showCron: false })}
                onOk={cron => this.updateAnswers('schedule', cron)}
            />);
        } else {
            return null;
        }
    ...
    renderOneParam(param, i) {
        return (<TableRow key={i} className={this.props.classes.tableRow}>
            ...
            <TableCell className={this.props.classes.paramsButtons}>
                <Fab className={this.props.classes.buttonDel} size="small" color="primary"
                     onClick={() => this.onDeleteParam(i)}
                     aria-label="Delete"><DeleteIcon/></Fab>
            </TableCell>
        </TableRow>);
    }
    ...
    renderStepSettings() {
        return [
            (<Fab className={this.props.classes.buttonAdd} size="small" color="secondary"
                  onClick={() => this.onAddParam()}
                  aria-label="Check"><AddIcon/></Fab>),
    ...
    renderNextButton(steps) {
        if (this.state.activeStep === steps.length - 1) {
            return this.state.requesting ?
                ...
                (<Fab className={this.props.classes.buttonCheck} size="small" color="secondary" variant="extended"
                      onClick={() => this.onCreate()} aria-label="Check"><CheckIcon/>Generate</Fab>);
        } else {
        ...
        return (
            <div className={this.props.classes.body}>
                <AppBar position="static" color="primary">
                    <Toolbar>
                        {this.state.screenWidth > NARROW_WIDTH ? (
                            <h4 className={this.props.classes.toolbarTitle}>Adapter creator</h4>) : null}
                    </Toolbar>
                </AppBar>
